-- Funções para lazy-load de totais e produtos (Vendas por Curva)

-- Totais por depto (nível 3/2/1), sem produtos
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    INNER JOIN dept3_totais dt ON b.dept3_nome = dt.dept3_nome
    ORDER BY
      b.dept3_nome,
      b.dept2_nome,
      b.dept1_nome
  ', p_schema, p_schema, p_schema, p_schema, p_schema, p_schema, p_schema, p_schema)
  USING p_ano, p_mes, p_filial_ids, p_page_size, v_offset, v_prev_fim;
END;
$function$;

ALTER FUNCTION public.get_venda_curva_totais(text, integer, integer, bigint[], integer, integer, date)
  SET search_path = public;

GRANT EXECUTE ON FUNCTION public.get_venda_curva_totais(text, integer, integer, bigint[], integer, integer, date)
  TO authenticated;

-- Produtos por depto (lazy-load)
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    produtos AS (
      ...
      WHERE COALESCE(d3.descricao, ''Sem Nível 3'') = $4
        AND COALESCE(d2.descricao, ''Sem Nível 2'') = $5
        AND d1.descricao = $6
        AND (
          $9 IS NULL
          OR p.id::text ILIKE $9
          OR p.descricao ILIKE $9
        )
    )
    SELECT
      pr.dept3_nome::text,
      ...
      CASE
        WHEN pr.total_valor_vendas > 0
        THEN ROUND((pr.total_lucro / pr.total_valor_vendas) * 100, 2)
        ELSE 0
      END as percentual_lucro,
      pr.curva_venda::text,
      pr.curva_lucro::text,
      ...
      CASE
        WHEN pr.total_valor_vendas_prev > 0
        THEN ROUND((pr.total_lucro_prev / pr.total_valor_vendas_prev) * 100, 2)
        ELSE 0
      END as percentual_lucro_ano_anterior
    FROM produtos pr
    ORDER BY
    ...
    LIMIT $10 OFFSET $11
  ', p_schema, p_schema, p_schema, p_schema, p_schema, p_schema, p_schema, p_schema)
  USING p_ano, p_mes, p_filial_ids, p_dept_nivel3, p_dept_nivel2, p_dept_nivel1, v_prev_fim, v_prev_inicio, p_search, p_page_size, v_offset;
END;
$function$;

ALTER FUNCTION public.get_venda_curva_produtos(text, integer, integer, bigint[], text, text, text, integer, integer, date, text)
  SET search_path = public;

GRANT EXECUTE ON FUNCTION public.get_venda_curva_produtos(text, integer, integer, bigint[], text, text, text, integer, integer, date, text)
  TO authenticated;
