-- =========================================================
-- Hardening: Produtos sem vendas + exclusividade de setores
-- =========================================================

-- 1) Reescreve a função principal com:
--    - filtro robusto sem LIMIT arbitrário em produtos_base
--    - suporte a vendas_hoje_itens no cálculo da última venda
--    - parâmetros parseados para arrays tipados
...
BEGIN
    ...
    produtos_base AS (
      ...
      WHERE p.ativo = true
        ...
        AND (
          $3 <> ''departamento''
          OR $4 IS NULL
          OR p.departamento_id = ANY($4)
        )
        AND (
          $3 <> ''setor''
          OR $4 IS NULL
          OR p.departamento_id IN (SELECT sd.departamento_id FROM setores_departamentos sd)
        )
        AND (
          $3 <> ''produto''
          OR $5 IS NULL
  ...
  RETURN QUERY EXECUTE v_query
    USING
      ...
      p_curva_abc,          -- $2
      p_filtro_tipo,        -- $3
      v_departamentos_arr,  -- $4 (departamento_ids ou setor_ids, depende do tipo)
      v_produtos_arr,       -- $5
      v_data_limite_min,    -- $6
      v_data_limite_max,    -- $7
      NULL,                 -- $8 reservado
...
-- 2) Trigger de exclusividade de departamentos entre setores ativos
--    (cria automaticamente em todos os schemas de tenant que possuam tabela setores)
DO $$
DECLARE
  r record;
BEGIN
  ...
  LOOP
    EXECUTE format($fn$
      ...
      BEGIN
        ...
        IF NEW.departamento_ids IS NULL OR cardinality(NEW.departamento_ids) = 0 THEN
          RETURN NEW;
        END IF;

        SELECT array_agg(DISTINCT dep)
        INTO v_overlap
        FROM %1$I.setores s
        CROSS JOIN LATERAL unnest(s.departamento_ids) dep
        WHERE s.ativo = true
          AND s.id <> COALESCE(NEW.id, -1)
          AND dep = ANY(NEW.departamento_ids);

        IF v_overlap IS NOT NULL THEN
          RAISE EXCEPTION
            'Conflito de setor: departamentos %% já pertencem a outro setor ativo',
            v_overlap
            USING ERRCODE = '23514';
        END IF;
    ...
    EXECUTE format('DROP TRIGGER IF EXISTS trg_setores_bloquear_departamento_duplicado ON %I.setores', r.table_schema);
    EXECUTE format(
      'CREATE TRIGGER trg_setores_bloquear_departamento_duplicado
       BEFORE INSERT OR UPDATE OF departamento_ids, ativo
       ON %I.setores
       FOR EACH ROW
       EXECUTE FUNCTION %I.trg_setores_bloquear_departamento_duplicado()',
      r.table_schema,
      r.table_schema
    );
  END LOOP;
...
-- Sem limpeza pontual de tenant específico.
