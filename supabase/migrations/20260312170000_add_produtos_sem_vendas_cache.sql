-- =========================================================
-- Fase 3: cache de ultima venda/entrada por produto e filial
-- =========================================================
...
CREATE OR REPLACE FUNCTION public.refresh_produtos_sem_vendas_cache(
  p_schema text,
...
BEGIN
  EXECUTE format('
    CREATE TABLE IF NOT EXISTS %1$I.ultima_venda_produto_filial (
  ...
  EXECUTE format('
    CREATE INDEX IF NOT EXISTS %1$I
    ON %2$I.ultima_venda_produto_filial (filial_id, data_ultima_venda DESC, produto_id)',
    'idx_ultima_venda_produto_filial_lookup',
    p_schema
  );

  IF p_filial_id IS NULL THEN
    EXECUTE format('TRUNCATE TABLE %I.ultima_venda_produto_filial', p_schema);
  ELSE
    EXECUTE format('DELETE FROM %I.ultima_venda_produto_filial WHERE filial_id = $1', p_schema)
    USING p_filial_id;
  END IF;

  EXECUTE format('
    INSERT INTO %1$I.ultima_venda_produto_filial (
      produto_id,
      filial_id,
      data_ultima_venda,
      data_ultima_entrada,
    ...
    WITH vendas_hist AS (
      SELECT
        ...
        MAX(v.data_venda)::date AS data_ultima_venda
      FROM %1$I.vendas v
      WHERE ($1 IS NULL OR v.filial_id = $1)
    ...
    vendas_hoje AS (
      SELECT
        ...
        MAX(vhi.data_extracao)::date AS data_ultima_venda
      FROM %1$I.vendas_hoje_itens vhi
      WHERE ($1 IS NULL OR vhi.filial_id = $1)
        AND vhi.cancelado = false
    ...
    todas_vendas AS (
      SELECT
        tv.produto_id,
        tv.filial_id,
        MAX(tv.data_ultima_venda)::date AS data_ultima_venda
      FROM (
        SELECT vh.produto_id, vh.filial_id, vh.data_ultima_venda FROM vendas_hist vh
        UNION ALL
        SELECT vho.produto_id, vho.filial_id, vho.data_ultima_venda FROM vendas_hoje vho
      ) tv
      GROUP BY tv.produto_id, tv.filial_id
    ),
    ultima_entrada AS (
      SELECT DISTINCT ON (e.filial_id, ep.produto_id)
        ep.produto_id,
        e.filial_id,
        e.data_entrada::date AS data_ultima_entrada
      FROM %1$I.entradas_produtos ep
      INNER JOIN %1$I.entradas e
        ON e.id = ep.entrada_id
      WHERE ($1 IS NULL OR e.filial_id = $1)
      ORDER BY e.filial_id, ep.produto_id, e.data_entrada DESC, e.id DESC
    )
    SELECT
      tv.produto_id,
      tv.filial_id,
      tv.data_ultima_venda,
      ue.data_ultima_entrada,
      now()
    FROM todas_vendas tv
    LEFT JOIN ultima_entrada ue
      ON ue.produto_id = tv.produto_id
     AND ue.filial_id = tv.filial_id
  ', p_schema)
  USING p_filial_id;
END;
$function$;
