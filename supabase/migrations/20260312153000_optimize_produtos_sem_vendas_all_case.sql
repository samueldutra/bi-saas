-- =========================================================
-- Fase 2.2: otimiza caminho "all" por filial
-- =========================================================
  ...
  IF v_is_all_case THEN
    v_query := format('
      ...
      ultimas_vendas_historico AS (
        SELECT DISTINCT ON (v.filial_id, v.id_produto)
          v.filial_id,
          v.id_produto AS produto_id,
          v.data_venda::date AS data_ultima_venda
        FROM %1$I.vendas v
        WHERE ($1 IS NULL OR v.filial_id = ANY($1))
        ORDER BY v.filial_id, v.id_produto, v.data_venda DESC
      ),
      ultimas_vendas_hoje AS (
        SELECT DISTINCT ON (vhi.filial_id, vhi.produto_id)
          vhi.filial_id,
          vhi.produto_id,
          vhi.data_extracao::date AS data_ultima_venda
        FROM %1$I.vendas_hoje_itens vhi
        WHERE ($1 IS NULL OR vhi.filial_id = ANY($1))
          AND vhi.cancelado = false
        ORDER BY vhi.filial_id, vhi.produto_id, vhi.data_extracao DESC
      ),
      todas_ultimas_vendas AS (
        ...
        FROM (
          SELECT
            vh.filial_id,
            vh.produto_id,
            vh.data_ultima_venda
          FROM ultimas_vendas_historico vh
          UNION ALL
          SELECT
            vho.filial_id,
            vho.produto_id,
            vho.data_ultima_venda
          FROM ultimas_vendas_hoje vho
        ) uv
...
BEGIN
  ...
  LOOP
    -- Indices pesados removidos desta migration.
    -- Criar manualmente com CONCURRENTLY fora de transacao.
  END LOOP;
END $$;
