-- =========================================================
-- Fase 2.3: reduzir janela de leitura de vendas no caso "all"
-- Regra de negocio: so interessam produtos cuja ultima venda esteja
-- entre data_ref - dias_max e data_ref - dias_min.
-- Logo, nao precisamos varrer historico anterior a data_limite_min.
-- =========================================================
    ...
    v_query := format('
      ...
      todas_ultimas_vendas AS (
        ...
        FROM (
          ...
          FROM vendas_no_periodo vh
          UNION ALL
          SELECT
            vho.filial_id,
            vho.produto_id,
            vho.data_ultima_venda
          FROM vendas_hoje_no_periodo vho
        ) uv
