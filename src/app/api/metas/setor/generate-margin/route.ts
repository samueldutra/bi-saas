...
export async function POST(request: NextRequest) {
  try {
    ...
    const filialId = parseInt(String(filial_id), 10)
    const setorId = parseInt(String(setor_id), 10)

    if (isNaN(filialId) || isNaN(setorId)) {
      return NextResponse.json({ error: 'setor_id ou filial_id inválido' }, { status: 400 })
    }

    if (authorizedBranches !== null && !authorizedBranches.includes(String(filialId))) {
      return NextResponse.json(
    ...
    if (existingMetasError) {
      console.error('[API/METAS/SETOR/GENERATE-MARGIN] Error checking existing metas:', existingMetasError)
      return NextResponse.json(
    ...
    if ((existingMetas?.length ?? 0) === 0) {
      ...
      if (rpcError && useFaturamentoMetas) {
        const fallback = await rpcClient.rpc('generate_metas_setor', {
          p_schema: schema,
          p_setor_id: setorId,
          p_filial_id: filialId,
          p_mes: mes,
      ...
      if (rpcError) {
        console.error('[API/METAS/SETOR/GENERATE-MARGIN] Error creating metas:', rpcError)
        return NextResponse.json(
          { error: 'Erro ao gerar meta de margem do setor' },
          { status: 500 }
        )
      }
      ...
      return NextResponse.json({
        ...
        success: true,
        message: 'Meta de margem do setor gerada com sucesso para o período',
      })
    }
    ...
    if (error) {
      console.error('[API/METAS/SETOR/GENERATE-MARGIN] Error:', error)
      return NextResponse.json(
        { error: 'Erro ao gerar meta de margem do setor' },
        { status: 500 }
      )
    ...
    console.error('[API/METAS/SETOR/GENERATE-MARGIN] Unexpected error:', error)
    return NextResponse.json(
